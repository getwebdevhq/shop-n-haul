...
export default function HomeClient({
  ...
  const heroContent = heroBlock?.content || {
    subtitle: "Jewellery Designed For Every Moment",
    image_url: "/images/hero_jewellery.png",
    button_text: "Shop Collections",
    button_link: "#categories",
  };

  // Get Banner Content
  const bannerContent = bannerBlock?.content || {
    subtitle: "The Signature Collection",
    description: "We curate minimal everyday pieces, cast in 18ct yellow gold vermeil and solid sterling silver. Zero compromise, high precision design. Stack them, stack yours, make it custom.",
    button_text: "Explore Collection",
    button_link: "#collections",
    image_url: "/images/rings_stack.png",
  };

  // Get Testimonials Content
  const testimonialsList = testimonialsBlock?.content || [];

  // Get Newsletter Content
  const newsletterContent = newsletterBlock?.content || {
    heading: "Join the Muse Club",
    description: "Subscribe for early access to new collection launches, exclusive events, and 10% off your first order.",
  ...
  // SEO Product Catalog Schema (INR)
  const catalogSchema = {
    "@context": "https://schema.org",
    "@type": "ItemList",
    ...
    "itemListElement": products.map((prod, idx) => ({
      ...
      "item": {
        "@type": "Product",
        "image": prod.product_images?.[0]?.image_url 
          ? `https://stashandhaul.vercel.app${prod.product_images[0].image_url}`
          : "https://stashandhaul.vercel.app/images/logo.png",
        "name": prod.name,
        "offers": {
          "@type": "Offer",
          "price": prod.price,
    ...
    <div className="relative min-h-screen bg-ivory text-charcoal overflow-x-hidden selection:bg-stone selection:text-charcoal font-sans">
      ...
      >
        ...
        >
          ...
          >
            {heroBlock?.title || "TIMELESS JEWELLERY"}
          </motion.h1>
          ...
          >
            <MagneticButton>
              <Link
                href={heroContent.button_link || "#categories"}
                className="px-8 py-4 bg-ivory text-charcoal hover:bg-gold hover:text-charcoal font-semibold text-xs uppercase tracking-[0.2em] transition-colors duration-500 shadow-xl flex items-center gap-2 rounded-theme"
              >
        ...
        >
          {categories.map((cat, idx) => (
            <motion.div
              key={cat.id}
              variants={fadeInUp}
              className="group relative overflow-hidden bg-stone/20 aspect-[3/4] cursor-pointer rounded-theme"
            >
              {cat.image_url && (
                <Image
                  src={cat.image_url}
      ...
      <section id="best-sellers" className="py-24 bg-stone/10 border-t border-b border-stone/30 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 overflow-x-auto no-scrollbar flex space-x-8 pb-10 cursor-grab active:cursor-grabbing">
          {products
            .filter((p) => p.bestseller || p.tags?.includes("Best Seller") || p.tags?.includes("Trending"))
            .map((product) => (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          ...
          >
            ...
            <h2 className="font-display text-5xl md:text-7xl font-light tracking-wide leading-tight text-charcoal">
              {bannerBlock?.title || "Designed to be worn every day."}
            </h2>
            ...
            <div className="pt-4">
              <MagneticButton>
                <Link
                  href={bannerContent.button_link || "#collections"}
                  className="px-8 py-4 bg-charcoal text-ivory hover:bg-gold hover:text-charcoal font-semibold text-xs uppercase tracking-[0.2em] transition-colors duration-500 shadow-xl rounded-theme"
                >
          ...
          <motion.div
            ...
            className="relative aspect-[3/4] w-full bg-stone/20 overflow-hidden shadow-2xl rounded-theme"
          >
      ...
      <section className="py-24 bg-stone/20 border-t border-b border-stone/30">
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {products
              .filter((p) => 
                p.tags?.includes(activeTrend) || 
                (activeTrend === "Gold Hoops" && p.tags?.includes("Gold") && p.tags?.includes("Earrings")) ||
                (activeTrend === "Pearl Edit" && p.tags?.includes("Pearl"))
              )
              .slice(0, 4)
              .map((product) => (
      ...
      {/* ======================================================== */}
      <HamperBuilder products={products} />
      ...
      <section id="new-arrivals" className="py-24 bg-stone/5 border-t border-b border-stone/30">
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {products
            .filter((p) => p.new_arrival || p.tags?.includes("New Arrival"))
            .slice(0, 4)
            .map((product) => (
              <ProductCard key={product.id} product={product} />
            ))}
      ...
      <section id="gifts" className="py-32 px-6 md:px-12 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {/* Gifts Under £50 */}
          <div className="group relative aspect-[3/4] bg-stone/20 overflow-hidden cursor-pointer flex flex-col justify-end p-8 text-ivory rounded-theme">
            <Image
              src="/images/rings_stack.png"
              alt="Gifts Under ₹5,000"
              fill
            ...
            <div className="relative z-10 text-left">
              ...
              <h3 className="font-display text-3xl font-medium tracking-wide">Gifts Under {formatCurrency(5000)}</h3>
          ...
          <div className="group relative aspect-[3/4] bg-stone/20 overflow-hidden cursor-pointer flex flex-col justify-end p-8 text-ivory rounded-theme">
            ...
            <div className="relative z-10 text-left">
              ...
              <h3 className="font-display text-3xl font-medium tracking-wide">Gifts Under {formatCurrency(10000)}</h3>
              <p className="text-xs text-stone/80 mt-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
      ...
      <section className="py-24 bg-stone/10 border-t border-b border-stone/30">
        <div className="max-w-7xl mx-auto px-6 md:px-12 text-center mb-16 space-y-4">
          <span className="text-[10px] tracking-[0.3em] uppercase text-gold font-semibold font-display">Community Lookbook</span>
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <div key={idx} className="group relative aspect-square bg-stone/20 overflow-hidden cursor-pointer shadow-sm rounded-theme">
              <Image
                src={img}
      ...
      {/* ======================================================== */}
      {/* SECTION 10: BRAND STORY & TESTIMONIALS                   */}
      {/* ======================================================== */}
      {testimonialsList.length > 0 && (
        <section className="py-24 bg-stone/5 border-t border-b border-stone/30">
          <div className="max-w-4xl mx-auto px-6 text-center space-y-12">
            <span className="text-[10px] tracking-[0.3em] uppercase text-gold font-bold">What Muses Say</span>
            
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {testimonialsList.map((t: any, idx: number) => (
                <div key={idx} className="space-y-4 text-left p-6 border border-stone/10 bg-ivory/50 rounded-theme">
                  <div className="text-gold text-sm">{"★".repeat(t.rating)}</div>
                  <p className="text-charcoal/70 text-xs italic leading-relaxed">&ldquo;{t.content}&rdquo;</p>
                  <div>
                    <h4 className="font-semibold text-xs text-charcoal">{t.name}</h4>
                    <span className="text-[9px] text-stone/50 uppercase tracking-widest">{t.role}</span>
                  </div>
                </div>
              ))}
            </div>
      ...
      {/* BRAND STORY GUARANTEE */}
      <section className="py-36 bg-charcoal text-ivory text-center relative overflow-hidden">
        <div className="max-w-4xl mx-auto px-6 space-y-8 relative z-10">
          <span className="text-[10px] tracking-[0.3em] uppercase text-gold font-bold">The LuxeCommerce Guarantee</span>
      ...
      {/* Newsletter */}
      <section className="py-24 bg-[#1e1e1e] text-ivory text-center">
        <div className="max-w-md mx-auto px-6 space-y-6">
          ...
          <div className="flex gap-2 max-w-xs mx-auto pt-2">
            <input
              ...
              className="bg-charcoal border border-stone/25 px-3 py-2 text-xs text-ivory flex-grow focus:outline-none focus:border-gold"
            />
            <button className="px-4 py-2 bg-gold text-charcoal text-[9px] uppercase tracking-wider font-semibold hover:bg-ivory transition-colors duration-300">
